...
#code with autorun implemented 

pygame.init()

pygame.display.set_caption("HackRU Platformer")
...
#font = pygame.font.Font('dejavuserif')
...
def get_block(size):
    ...
    rect = pygame.Rect(0, 125, size, size)
...
class Player(pygame.sprite.Sprite):
    ...
    def loop(self, fps):
        if not self.hit:  # Only auto-run if not hit
...
def main(window):
    ...
    background, bg_image = get_background("platformer.jpg")

    block_size = 96
    player = Player(0, 100, 50, 50)
    fire = Fire(700, HEIGHT - block_size - 64, 16, 32)
    fire.on()
    fire1 = Fire(1500, HEIGHT - block_size - 64, 16, 32)
    fire1.on()
